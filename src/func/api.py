...
def extract_keywords(text: str, onlineCars: str) -> dict:
    prompt = f"""
        ...
        7. 如果輸入是簡短的"不要", "好", "拒絕"等簡短的字眼，請直接將此直接放入"傳達訊息"中
        8. 有時也會只有單純的對話，沒有任何其他車牌資訊。
        此時請你把該對話內容簡單化後放入"傳達訊息"
        並將傳給的車牌號碼設為空字串("")
        8. 不要有"```json"以及"```"，直接輸出json檔案內容
        9. 現在已知在現的車牌有：{onlineCars}，若需回傳車牌，請只能從這些號碼中選擇"傳給的車牌號碼"
        9. 通常輸入文字的最後端會有"我的車牌號碼是..."，請將該車牌號碼帶入"來自的車牌號碼"
        10. 回傳格式如下（不要有其他說明）：
        {{
        "correctness": "1",
        "來自的車牌號碼": "ABC1234",
        ...
        再次提醒，若需回傳車牌，你輸出的"傳給的車牌號碼"必須在{onlineCars}之中。
